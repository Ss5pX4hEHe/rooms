  ...
  return (
    <div className="flex-1 overflow-y-auto py-2" onClick={() => setMenuId(null)}>
      {announcements.length > 0 && (
        <div className="mb-1">
          {announcements.map(renderChat)}
        </div>
      )}

      {directs.length > 0 && (
        <div className="mb-1">
          <p className="px-5 py-2 text-[11px] font-semibold text-tx2 uppercase tracking-wider">Direct Messages</p>
          {directs.map(renderChat)}
        </div>
      )}

      {groups.length > 0 && (
        <div className="mb-1">
          <p className="px-5 py-2 text-[11px] font-semibold text-tx2 uppercase tracking-wider">Groups</p>
          {groups.map(renderChat)}
        </div>
      )}
    </div>
  );
