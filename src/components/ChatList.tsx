  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="absolute right-0 top-0 bottom-0 flex items-center pr-3" style={{ width: 80 }}>
        {showConfirm ? (
          <button onClick={() => { onDelete(); cancel(); }}
            className="w-full h-[calc(100%-8px)] rounded-xl bg-red-500 text-white text-xs font-semibold flex items-center justify-center active:bg-red-600">
            Delete
          </button>
        ) : (
      ...
      {/* Chat content */}
      <div ref={ref} style={{ transform: `translateX(-${offset}px)`, transition: swiping.current ? "none" : "transform 0.2s ease-out" }}
  ...
  function renderChat(c: typeof chats[0], isLast: boolean) {
      ...
      <div className="px-2">
        <button onClick={() => { setMenuId(null); router.push(`/chat/${c.chat_id}`); }}
          ...
          className={`w-full flex items-center gap-3 px-3 py-3 text-left rounded-xl transition-all duration-150
            ${active ? "bg-pri/10" : "hover:bg-surface active:bg-surface-h"}`}>
    ...
    return (
      <div key={c.chat_id} className="relative group">
        {/* Mobile: swipe to delete. Desktop: right click */}
        <div className="md:hidden">
          <SwipeChat onDelete={async () => { await deleteChat(c.chat_id); if (active) router.push("/chat"); }}>
            {chatButton}
          </SwipeChat>
        </div>
