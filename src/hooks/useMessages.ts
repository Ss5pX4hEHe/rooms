...
export function useMessages(chatId: string | null) {
  const { user, setMessages, addMessage, updateMessage, removeMessage, updateLastMsg, markMessagesRead, clearUnread, setTypingUsers, setPinnedMessages, showToast } = useStore();
  ...
  const loadMessages = useCallback(async () => {
    ...
    const msgs: Message[] = (data || []).map((m: any) => {
      ...
      return {
        ...
        content: m.deleted ? "" : m.content, status: m.status || "sent", created_at: m.created_at,
  ...
  const loadPinned = useCallback(async () => {
    ...
    setPinnedMessages((data || []).map((p: any) => ({
  ...
  // Mark others' messages as read in DB + broadcast
  const markAsRead = useCallback(async () => {
    ...
    if (updated && updated.length > 0) {
      ...
      chRef.current?.send({ type: "broadcast", event: "message:read", payload: { reader_id: user.id, message_ids: ids } });
    }
  }, [chatId, user, markMessagesRead]);

  useEffect(() => {
    if (!chatId || !user) return;
    loadMessages();
    loadPinned();
    // Update last_read + last_seen
    ...
    clearUnread(chatId);
    // Mark messages as read after they load
    setTimeout(() => markAsRead(), 600);
    ...
    const ch = supabase.channel(`chat:${chatId}`)
      .on("broadcast", { event: "message:new" }, ({ payload }) => {
        ...
        if (msg.sender_id !== user.id) {
          // We're in the chat, mark as read immediately
          clearUnread(chatId);
          supabase.from("chat_members").update({ last_read_at: new Date().toISOString() }).eq("chat_id", chatId).eq("user_id", user.id).then();
      ...
      .on("broadcast", { event: "message:read" }, ({ payload }) => {
        // Other person read our messages — show ✓✓
        if (payload.reader_id !== user.id && payload.message_ids) {
          markMessagesRead(payload.message_ids);
        }
      })
      ...
      .on("broadcast", { event: "typing" }, ({ payload }) => {
        if (payload.user_id !== user.id) {
          ...
          if (!current.includes(payload.username)) setTypingUsers(chatId, [...current, payload.username]);
          setTimeout(() => {
  ...
  const pinMessage = useCallback(async (msgId: string) => {
    if (!chatId || !user) return;
    const store = useStore.getState();
    if (store.pinnedMessages.length >= 5) { showToast("Max 5 pinned"); return; }
    ...
    await loadPinned(); showToast("Pinned");
  ...
  const unpinMessage = useCallback(async (pinId: string) => {
    ...
    chRef.current?.send({ type: "broadcast", event: "pin:update", payload: {} });
    await loadPinned(); showToast("Unpinned");
  }, [loadPinned]);
